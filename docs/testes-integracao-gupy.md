...
| **Passos** | `curl -H "Authorization: Bearer $TOKEN" $BASE/companies/jobs?perPage=50` ou `pnpm --filter @collab/api exec ts-node scripts/probe-listar-vagas.ts`. |
...
pnpm --filter @collab/api test:cov

# Integração local (com docker-compose up)
pnpm --filter @collab/api test:int

# Probes contra sandbox (manuais, exigem credenciais)
pnpm --filter @collab/api exec ts-node scripts/probe-listar-vagas.ts
pnpm --filter @collab/api exec ts-node scripts/emitir-webhook.ts application.created
```
